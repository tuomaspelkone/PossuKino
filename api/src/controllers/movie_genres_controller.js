...
export async function getMovie_genre(req, res, next) {
  try {
    const movie_genre = await getOne(req.params.tmdb_id);
    if (!movie_genre) {
...
export async function updateMovie_genre(req, res, next) {
  try {
    const response = await updateOne(req.params.tmdb_id, req.body);
    res.json(response);
  } catch (err) {
    next(err);
  }
...
export async function deleteMovie_genre(req, res, next) {
  try {
    const movie_genre = await deleteOne(req.params.tmdb_id);
    if (!movie_genre) {
      return res.status(404).json({ error: "Movie_genre not found" });
    }
    res.json(movie_genre);
