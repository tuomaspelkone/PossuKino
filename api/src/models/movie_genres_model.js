...
export async function addOne(movie_genres) {
  const result = await pool.query(
    "INSERT INTO movie_genres (movie_id,genre_id) VALUES($1,$2) RETURNING *",
    [movie_genres.movie_id, movie_genres.genre_id]
  );
  return result.rows;
}
...
export async function updateOne(id,movie_genres) {
  ...
  const result = await pool.query(
    "UPDATE movie_genres SET movie_id=$1, genre_id=$2 WHERE movie_id=$3 RETURNING *",
    [movie_genres.movie_id, movie_genres.genre_id, id]
  );
  return result.rows;
}
